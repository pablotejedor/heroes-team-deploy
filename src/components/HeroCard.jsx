...
export const HeroCard = ({ hero, heroTeam, setHeroTeam }) => {
  const goodGuysCount = heroTeam.filter(
    (selectedHero) => selectedHero.biography.alignment === "good"
  ).length;
  const badGuysCount = heroTeam.filter(
    (selectedHero) => selectedHero.biography.alignment === "bad"
  ).length;
  ...
  return (
    ...
    >
      <img
        // className="col-5"
        src={hero.image.url || 'https://via.placeholder.com/206x174.png?text=No+Image'}
        alt={hero.id}
